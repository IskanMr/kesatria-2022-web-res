...
const titleClass =
    'text-black text-2xl md:text-4xl font-gill font-bold mb-2 md:mb-4';
const descClass = 'md:text-lg mb-4 md:mb-6';

export function Material1() {
    return (
        <>
            <div className={titleClass}>Personal Branding</div>
            <div className={descClass}>
                ...
                bagaimana cara melakukan Personal Branding yang baik, seperti
                membuat CV dan resume yang benar. Selain itu, Kesatria juga akan
                dikenalkan dengan Applicant Tracking System (ATS) dan aplikasi
                LinkedIn.
            </div>
...
export function Material2() {
    return (
        <>
            <div className={titleClass}>Fasilitas Fakultas Teknik</div>
            <div className={descClass}>
                Melalui materi Fasilitas Fakultas Teknik, Kesatria Muda akan
                dikenalkan dengan berbagai fasilitas di Fakultas Teknik yang
                menunjang kegiatan akademik maupun non-akademik.
                Fasilitas-fasilitas tersebut terdiri dari fasilitas fisik dan
                fasilitas non-fisik. Di akhir modul juga terdapat pedoman
                informasi dan tutorial penggunaan fasilitas yang terdapat di
                Fakultas Teknik yang terdapat pada Buku Pelayanan.
            </div>
            <div className='xl:h-xl lg:h-xl md:h-xl sm:h-lg 320px:h-md'>
                <iframe
...
export function Material3() {
    const dropDownsStyle =
        'flex-row px-1 bg-white text-oranges-default cursor-pointer py-1 w-full hover:bg-oranges-default hover:text-white justify-center place-content-center transition ease-in-out duration-300';
    const dropDownsActive =
        'flex-row bg-oranges-default px-1 text-white cursor-pointer py-1 w-full justify-center place-content-center transition ease-in-out duration-300';
    const jurusan = [
    ...
    const dropDownsShow =
        'show bg-oranges-default w-full overflow-auto mx-auto z-10 absolute flex flex-col transition ease-in-out duration-300';
    const dropDownsHidden =
        'hidden bg-oranges-default w-2/3 overflow-auto mx-auto z-10 absolute transition ease-in-out duration-300';
    ...
    jurusan.forEach((item) => {
        if (
            ...
            `/materi/Pengetahuan-Program-Studi/${item.value}`
        ) {
            rightNow = item.value;
        }
    });
    ...
    return (
        <>
            <div className={titleClass}>Pengetahuan Program Studi</div>
            <div className={descClass}>
                Materi Pengetahuan Program Studi akan menjelaskan tentang
                hal-hal mengenai 14 program studi yang ada di Fakultas Teknik.
                Materi ini berisi penjelasan mengenai visi misi program studi,
                apa saja yang akan dipelajari pada program studi tersebut,
                berbagai fasilitas dan laboratorium, media sosial dan laman
                resmi program studi, hingga prospek kerja dari program studi
                terkait.
            </div>
            <Router>
                <div className='w-full mx-auto mb-4'>
                    <div className='w-full relative inline-block mx-auto'>
                        <button
                            id='dropDownsID'
                            onClick={() => {
                                if (tampilDropdowns === dropDownsShow) {
                                    tampilkanDrop(dropDownsHidden);
                                } else {
                                    tampilkanDrop(dropDownsShow);
                                }
                            }}
                            type='button'
                            className='bg-oranges-default flex-row justify-between px-1 text-white cursor-pointer border-2 py-1 w-full rounded-md'
                        >
                            {dropDownsValue}
                            <svg
                                className='h-5 w-5 inline-flex justify-items-end'
                        ...
                        <div id='myDropdown' className={tampilDropdowns}>
                            {jurusan.map((item) => (
                                <NavLink
                                    ...
                                    activeClassName={dropDownsActive}
...
export function Material4() {
    return (
        <>
            <div className={titleClass}>Karya Aplikatif</div>
            <div className={descClass}>
...
export function Material5() {
    return (
        <>
            <div className={titleClass}>Jalan-Jalan Teknik</div>
            {/* VIDEO PLAYER */}
            {/* <div className=''>
            ...
            <div className={descClass}>COMING SOON</div>
        </>
    );
}
...
export function Material6() {
    return (
        <>
            ...
            <div className={descClass}>
                <p>
                    Fakultas Teknik memiliki sejarah yang sangat panjang.
                    Melalui materi Sejarah Teknik, Kesatria Muda akan diajak
                    untuk menonton video animasi yang akan memaparkan tentang
                    perjalanan sejarah Fakultas Teknik dari awal dibentuk hingga
                    menjadi fakultas terbesar di Universitas Gadjah Mada seperti
                    saat ini.
                </p>
            </div>
...
export function Material7() {
    return (
        <>
            <div className={titleClass}>Swakarya Kampus</div>
            <div className={descClass}>
                Materi Swakarya Kampus akan digunakan sebagai bahan bacaan
                sebelum melakukan kegiatan Swakarya Kampus pada hari ke-2 PPSMB
                Kesatria. Materi ini berisi referensi dan penjelasan tentang
                teknologi, barang, hingga desain perancangan tempat yang modern
                dan memiliki banyak manfaat. Referensi tersebut dibagi menjadi 3
                klaster, yaitu klaster manufaktur, infrastruktur, dan energi.
            </div>
            <div className='xl:h-xl lg:h-xl md:h-xl sm:h-lg 320px:h-md'>
                <iframe
